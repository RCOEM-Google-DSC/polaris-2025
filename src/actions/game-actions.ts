...
export const startNewGame = async () => {
  ...
  if (userData.gameState) {
    ...
    if (gameState.characters.every(char => char.guessed)) {
      ...
      gameState.currentIndex = gameState.characters.findIndex(char => !char.guessed);
    ...
    const shuffledCharacters = [...characters].sort(() => 0.5 - Math.random()).slice(0, 5).map(char => ({...char, guessed: false}));
...
export const validateGuess = async (guess: string) => {
  ...
  // Find the next unguessed character
  const nextUnguessedIndex = gameState.characters.findIndex((char, index) => index > gameState.currentIndex && !char.guessed);
